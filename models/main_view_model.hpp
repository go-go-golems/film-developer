...
public:
    ...
    static constexpr const char* PUSH_PULL_VALUES[] = {"-1", "0", "+1", "+2", "+3"};
    ...
    bool start_process() {
        ...
        FURI_LOG_I(
            MODEL_TAG,
            "Starting process, current state: %s",
            get_process_state_name(process_state));
    ...
    bool pause_process() {
        ...
        FURI_LOG_I(
            MODEL_TAG,
            "Pausing process, current state: %s",
            get_process_state_name(process_state));
        process_state = ProcessState::Paused;
        process_interpreter->pause();
        update();
        return true;
    }
    ...
    bool resume_process() {
        ...
        FURI_LOG_I(
            MODEL_TAG,
            "Resuming process, current state: %s",
            get_process_state_name(process_state));
        process_state = ProcessState::Running;
        process_interpreter->resume();
        update();
        return true;
    }
    ...
    bool wait_for_user() {
        ...
        FURI_LOG_I(
            MODEL_TAG,
            "Waiting for user confirmation, current state: %s",
            get_process_state_name(process_state));
    ...
    bool confirm_user_action() {
        ...
        FURI_LOG_I(
            MODEL_TAG,
            "User action confirmed, current state: %s",
            get_process_state_name(process_state));
        process_state = ProcessState::Running;
    ...
    void stop_process() {
        FURI_LOG_I(
            MODEL_TAG,
            "Stopping process, current state: %s",
            get_process_state_name(process_state));
        if(process_interpreter) {
    ...
    bool complete_process() {
        FURI_LOG_I(
            MODEL_TAG,
            "Process completed, current state: %s",
            get_process_state_name(process_state));
        reset_process_state();
        return true;
    }
