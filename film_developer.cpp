...
public:
    ...
    void send_custom_event(FilmDeveloperEvent event) {
        ...
        view_dispatcher_send_custom_event(view_dispatcher, static_cast<uint32_t>(event));
    }
...
private:
    ...
    void show_confirmation_dialog(ConfirmationDialogView::DialogType dialog_type) {
    ...
    bool handle_back_event() {
        ...
        auto model = this->model.lock();
        FURI_LOG_D(APP_TAG, "Model locked, Process active: %d", model->is_process_active());
    ...
    bool handle_custom_event(FilmDeveloperEvent event) {
        ...
        if(event == FilmDeveloperEvent::TimerTick) {
            FURI_LOG_T(
                APP_TAG, "Timer tick event, current state: %s", get_state_name(current_state));
        } else {
            FURI_LOG_D(
                APP_TAG,
        ...
        case FilmDeveloperEvent::RestartRequested:
            ...
            if(current_state == AppState::MainView || current_state == AppState::Paused ||
               current_state == AppState::DispatchDialog) {
                ...
                FURI_LOG_I(APP_TAG, "Restart requested, but not in a valid state, ignoring");
            }
            return true;
        ...
        case FilmDeveloperEvent::StopProcess:
            model->process_interpreter->stop();
            enter_state(AppState::ProcessSelection);
            return switch_to_view(ViewProcessSelection);
