...
public:
  ...
  ~FilmDeveloperApp() {
    if (view_dispatcher != nullptr) {
      FURI_LOG_D("FilmDev", "Freeing views");
      for (size_t i = 0; i < ViewCount; i++) {
        view_dispatcher_remove_view(view_dispatcher, static_cast<ViewId>(i));
        FURI_LOG_D("FilmDev", "Removed view %d", i);
      }
      view_dispatcher_free(view_dispatcher);
      FURI_LOG_D("FilmDev", "View dispatcher freed");
      furi_record_close(RECORD_GUI);
      FURI_LOG_D("FilmDev", "GUI freed");

      delete process_interpreter;
      FURI_LOG_D("FilmDev", "Process interpreter freed");
#ifndef HOST
      static_cast<MotorControllerEmbedded *>(motor_controller)->deinitGpio();
#endif
      delete motor_controller;
      FURI_LOG_D("FilmDev", "Motor controller freed");
    }
  }
    ...
    if (model->is_process_active() && !model->is_process_paused()) {
      ...
      if (!wasWaitingForUser && isWaitingForUser) {
        view_dispatcher_send_custom_event(
            view_dispatcher, static_cast<uint32_t>(FilmDeveloperEvent::UserActionRequired));
      }

      FURI_LOG_T(
          "FilmDev", "Process tick - Step: %d, Time: %ld/%ld",
...
private:
  ...
  static bool navigation_callback(void *context) {
    FURI_LOG_D("FilmDev", "Navigation callback");
    auto app = static_cast<FilmDeveloperApp *>(context);
  ...
  bool handle_back_event() {
    FURI_LOG_D("FilmDev", "Back event");
    auto model = this->model.lock();
    FURI_LOG_D("FilmDev", "Model locked, Process active: %d",
               model->is_process_active());
    switch (current_state) {
    case AppState::ProcessSelection:
      // Only allow exit to menu if no process is active
      if (!model->is_process_active()) {
        FURI_LOG_D("FilmDev", "No process active, stopping view dispatcher");
        view_dispatcher_stop(view_dispatcher);
        return true;
      }
      FURI_LOG_D("FilmDev", "Process active, staying in state");
      return false;
  ...
  bool handle_custom_event(FilmDeveloperEvent event) {
    auto model = this->model.lock();
    FURI_LOG_D("FilmDev", "Custom event received: %s, current state: %s",
    ...
    case FilmDeveloperEvent::UserActionRequired:
      // XXX not the cleanest way to do this, we should delegate entirely to the
      // process interpreter
    ...
    case FilmDeveloperEvent::RestartStep:
      if (current_state == AppState::MainView ||
        ...
        dialog_view.show_dialog(
            ConfirmationDialogView::DialogType::StepRestart);
        return switch_to_view(ViewConfirmationDialog);
      }
      return true;
