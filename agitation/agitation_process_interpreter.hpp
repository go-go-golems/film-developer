...
public:
    ...
    void init() override {
        initAgitation(&STAND_DEV_STATIC, motor_controller);
    }
    ...
    const char* getCurrentMovementName() const override {
        return motor_controller ? motor_controller->getDirectionString() : "Unknown";
    }

    void initAgitation(const AgitationProcessStatic* process, MotorController* motor_controller);
    bool tick() override;
    void reset() override;
    ...
    const AgitationProcessStatic* getCurrentProcess() const {
        return process;
    }
