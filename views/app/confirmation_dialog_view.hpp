...
public:
  ...
  void show_dialog(DialogType type) {
    FURI_LOG_D(CONFIRMATION_DIALOG_TAG, "Showing dialog, type: %d, %p",
...
private:
  ...
  void configure_for_restart() {
    ...
    set_text("Restart current step?\nTimer will reset.", 64, 32, AlignCenter,
             AlignCenter);
    set_left_button_text("Cancel");
  ...
  void configure_for_exit() {
    ...
    set_text("Exit application?\nAll progress will be lost.", 64, 32,
             AlignCenter, AlignCenter);
    set_left_button_text("Cancel");
  ...
  static void dialog_callback(DialogExResult result, void *context) {
    FURI_LOG_D(CONFIRMATION_DIALOG_TAG,
               "Dialog callback, result: %d, context: %p",
               static_cast<int>(result), context);
    auto view = static_cast<ConfirmationDialogView *>(context);
    if (result == DialogExResultRight) {
      switch (view->current_type) {
      case DialogType::ProcessStart:
        view->send_custom_event(
            static_cast<uint32_t>(FilmDeveloperEvent::StartProcess));
        break;
      case DialogType::ProcessAbort:
        view->send_custom_event(
            static_cast<uint32_t>(FilmDeveloperEvent::StopProcess));
        break;
      case DialogType::StepComplete:
        view->send_custom_event(
      ...
      case DialogType::StepRestart:
        view->send_custom_event(
            static_cast<uint32_t>(FilmDeveloperEvent::RestartStep));
        break;
      case DialogType::StepSkip:
        view->send_custom_event(
            static_cast<uint32_t>(FilmDeveloperEvent::SkipStep));
        break;
      case DialogType::AppExit:
        view->send_custom_event(
            static_cast<uint32_t>(FilmDeveloperEvent::ExitApp));
        break;
      }
    } else {
      view->send_custom_event(
